...
*Watermarks do not affect statistically sounding generated data, simply fail to pass the test. The "simply" adjective here is supported by the fact that every run provided more than 1% of the data set. Pushing a 1% of fibonacci series (or whatever not random) into the 6M+ of samples, will lead to the whole run.all fail the test. Instead pushing many different watermarks did not make the test fail. This shows that watermarks were detected as a "defects" only because circualarity of test evaluation, not becuase they repeat themselves in a way they are predictable. The watermarks of implementation in this scenario aren't anything else than a "peak of load" or "peculiar system conditions" that can happen during the generation without affecting the whole result nor the local acceptability. Under this PoV the watermarks just revealing that such numbers have been generated by a **class** of possible implementations, and that information leak is worth ONLY if that class of implementation is broken or the attacker can push the system conditions to lead the chaos engine providing predicatable sequences. This is the reason because such watermarks are seek and rejected when affecting the data with some kind of circularity.*
