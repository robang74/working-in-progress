...
- I credited myself to the AI as a software developer and since now, it will tend to relate with me in the proper manner to avoid the risk of seeing its work rejected. However, at the same time, I showed that I am open to accepting ideas and reframing pro-vs-cons in a third way. Thus, the AI mirroring will be pro and open as per default policy.
...
The AI continues to change the file in a way that it should be tested every time from zero again, potentially. So, it does not code in a git-patch compliant. Moreover, like Copilot also Gemini has the tendency to change the file in the whole (and this is because a LLM is made for generating language not to perform text maintenance, including code).
